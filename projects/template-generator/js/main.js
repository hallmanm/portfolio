...
function createTemplates(templates){
  if(templates){
    ...
    html += '<select id="templates" class="cs-text-box w-100" style="text-transform: capitalize;" name="templates" data-action="clear" onchange="updateTemplates(this)"><option value="">Custom</option>';

    for (var i = 0; i < templates.length; i++) {
      template = templates[i];

      html += '<option value="'+i+'">'+template.title+'</option>';
    }
    ...
    for (var i = 0; i < data.loops.breakpoints.length; i++) {
      ...
      switch(option){
        ...
        case 'justify':
          ...
          break;
        case 'padding':
          html += '<td class="'+option+'_'+breakpoint+'"><input id="'+option+'_'+breakpoint+'" data-type="'+option+'" class="cs-text-box w-100" type="number" min="0" max="100" value="0" name="'+option+'_'+breakpoint+'" onchange="updateOptions(this.getAttribute(\'data-type\'),\''+breakpoint+'\',parseInt(this.value))"/></td>';
          break;
        case 'border':
...
function updateTemplates(elem){
  ...
  if(elem){
    ...
    if(template.getElementsByTagName('option').length > 0){
      template.getElementsByTagName('option')[0].selected = 'selected';
      template.dataset.action = '';
    }
  }
}
//change breakpoint
...
function applyOptions(){
  if(!validate()){
    ...
    for (var i = 0; i < data.loops.breakpoints.length; i++) {
      ...
      for (var x = 0; x < data.loops.options.length; x++) {
        option = data.loops.options[x];
        blockOptions[breakpoint][option] = data.options[breakpoint][option];
      }
    }
